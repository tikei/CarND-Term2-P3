...
/* Calculates the probability density of a Multi Variate Normal Distribution
for two random variables x and y
with means mu_x , mu_y and
standard deviation sigma_x and sigma_y

NOTE: This function does not take into account covariances.
An upgrade to the function will include a Sigma var/ covar matrix implementation.

*/
inline double MultiVariateNormalPdf(double& x, double&y, double& mu_x, double& mu_y, double& sigma_x, double& sigma_y){

	double denomin = (2.0 * M_PI * sigma_x * sigma_y);
	double x_term = std::pow(x - mu_x, 2) / (2.0 * std::pow(sigma_x, 2));
	double y_term = std::pow(y - mu_y, 2) / (2.0 * std::pow(sigma_y, 2));

	// double numerator = std::exp( - ( std::pow(x-mu_x, 2) / (2.0 * std::pow(sigma_x, 2)) + (std::pow(y - mu_y, 2) / (2.0 * std::pow(sigma_y, 2))) ) );

	return std::exp(-(x_term + y_term)) / denomin;
